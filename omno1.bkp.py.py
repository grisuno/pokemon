...
class FastSlowLinear(nn.Module):
    """Linear layer con pesos hebbianos mejorados."""
    def __init__(self, in_features, out_features, fast_lr=0.01, fast_decay=0.95):
        ...
        # FIX: Inicializar con valores pequeños pero no cero
        ...
        self._update_count = 0  # Contador para debugging

    def reset_fast_weights(self):
        # FIX: Reset parcial en vez de total
        self.fast_weight *= 0.5  # Mantener algo de historia
        self.fast_bias *= 0.5
    ...
    def update_fast_weights(self, x: torch.Tensor):
        with torch.no_grad():
            ...
            # Computar activación actual
            slow_out = F.linear(x, self.slow_weight, self.slow_bias)
            
            # MEJORA: Hebbian update más robusto
            ...
            # MEJORA: Normalización adaptativa
            weight_norm = torch.norm(self.fast_weight, dim=1, keepdim=True)
            max_norm = 0.2  # Aumentado de 0.1
...
class ConsciousnessModule(nn.Module):
    """Módulo de consciencia con Φₑ mejorado y menos restrictivo."""
    def __init__(self, features: int, use_conscious=True):
        super().__init__()
        self.use_conscious = use_conscious
        self.integration_net = nn.Sequential(
            nn.Linear(features, features),
            nn.LayerNorm(features),  # MEJORA: Añadido LayerNorm
            nn.ReLU(),
            nn.Dropout(0.1),  # MEJORA: Regularización
            nn.Linear(features, features)
        )
        self.phi_effective = 0.0
        self.integration_threshold = 0.15  # Reducido de 0.2
        self.running_phi = 0.0
        self.phi_history = []  # Para debugging
        
    def compute_phi_effective(self, activity: torch.Tensor) -> float:
        """Φₑ mejorado con condiciones menos restrictivas."""
        # FIX: Condiciones menos estrictas
        if activity.numel() < 20 or activity.size(0) < 2:  # Era 100 y 5
            return 0.0
        ...
        with torch.no_grad():
            ...
            # MEJORA: Regularización para estabilidad numérica
            ...
            try:
                ...
                # MEJORA: Usar entropía de Shannon para diversidad
                ...
                # Suavizado exponencial más agresivo
                ...
                return self.running_phi  # Mantener valor anterior en caso de error
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # MEJORA: Integración más suave y continua
        if self.phi_effective > self.integration_threshold:
            ...
            # Siempre hay algo de integración
...
def train_model(model, train_loader, test_loader, device, epochs=15, lr=1e-3, 
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (x, y) in enumerate(train_loader):
            ...
            # NO resetear fast weights cada batch - dejar que acumulen
            if batch_idx == 0:
