...
class TopoBrainNetV18(nn.Module):
    ...
    def prune_topology(self):
        ...
        with torch.no_grad():
            ...
            adj_w = torch.sigmoid(self.adj_weights)
            important_edges = adj_w > self.config.prune_threshold
            ...
            for layer in [self.layer1, self.layer2]:
                if hasattr(layer, 'node_importance'):
                    ...
                    # Mantener al menos 50% de nodos
                    threshold = max(self.config.prune_threshold, 
                                  importance.quantile(0.5).item())
...
def train_epoch(model, train_loader, optimizer, opt_topo, supcon, config: Config, epoch: int, topo_monitor: TopologicalHealthSovereignty):
    """Entrena una época con schedule adaptativo de SupCon"""
    model.train()
    metrics = {'loss': 0, 'acc': 0, 'n_samples': 0, 'supcon_loss': 0, 'ortho_loss': 0}
    
    # Lambda dinámico para SupCon
    lambda_supcon = config.get_supcon_lambda(epoch)
    ...
    for batch_idx, (x, y) in enumerate(train_loader):
        ...
        ortho_loss_val = 0
        if config.use_plasticity:
            loss += config.lambda_sparsity * torch.norm(torch.sigmoid(model.adj_weights), 1)
    ...
    # Promediar
    metrics['loss'] /= metrics['n_samples']
    metrics['acc'] = 100 * metrics['acc'] / metrics['n_samples']
    metrics['supcon_loss'] /= metrics['n_samples']
    metrics['ortho_loss'] /= metrics['n_samples']
    metrics['lambda_supcon'] = lambda_supcon  # Guardar para logging
    ...
    if config.use_plasticity:
        ...
        if "PRUNED" in adj.status:
            ...
            # ACCIÓN AUTOMÁTICA (opcional pero potente)
