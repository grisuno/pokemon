...
class Config:
    ...
    # --- FIX 1: ESTABILIDAD DE FAST WEIGHTS ---
    fast_lr: float = 0.005       # Reducido para evitar explosión (antes 0.01)
    forget_rate: float = 0.6     # Aumentado para limpiar ruido (antes 0.95)
    # ------------------------------------------
    ...
    # --- FIX 2: RÉGIMEN HIGH-PERFORMANCE ---
    epochs: int = 150            # Aumentado para convergencia profunda
    lr_main: float = 0.001       # Learning rate base conservador
    lr_topo: float = 0.01
    topo_warmup_epochs: int = 10
    grad_clip_norm: float = 1.0
    accumulation_steps: int = 2
    
    weight_decay: float = 1e-3   # Aumentado para regularización (antes 1e-4)
    # ---------------------------------------
    ...
    lambda_supcon_start: float = 0.1      
    lambda_supcon_end: float = 1.0        
    lambda_entropy: float = 0.001
    lambda_sparsity: float = 1e-5
    lambda_ortho: float = 5e-3            
...
class PrefrontalOrchestrator(nn.Module):
    ...
    def forward(self, metrics_dict: Dict[str, float]) -> Dict[str, torch.Tensor]:
        """
        Orquestador v27: Allostasis con Frenado de Emergencia (Gradient-Aware).
        ...
        """
        # 1. EXTRACCIÓN DE MÉTRICAS DE SUPERVIVENCIA
        density = metrics_dict.get('density', 0.1)
        current_loss = metrics_dict.get('loss', 10.0)
        grad_norm = metrics_dict.get('grad_norm', 1.0)
        
        # --- PROTOCOLO DE FRENADO DE EMERGENCIA (CRISIS) ---
        # Si el loss se dispara por encima de 10.0, significa que estamos divergiendo.
        # Acción: Cortar dopamina (LR) y subir escudos.
        if current_loss > 10.0:
            return {
                'plasticity': torch.tensor(0.1, device=self.config.device),    # Mínima plasticidad
                'memory': torch.tensor(0.95, device=self.config.device),       # Aferrarse a memoria
                'defense': torch.tensor(0.9, device=self.config.device),       # Máxima defensa
                'supcon': torch.tensor(0.0, device=self.config.device),        # Apagar contrastive
                'symbiotic': torch.tensor(0.0, device=self.config.device),     # Apagar ortho
                'sparsity': torch.tensor(0.0, device=self.config.device),      # NO PODAR
                'lr_scale': torch.tensor(0.1, device=self.config.device)       # LR mínimo absoluto
            }
        
        # --- LÓGICA DE DENSIDAD (Modo Élite vs Supervivencia) ---
        is_weak = density < 0.05
        # Si el loss es bajo (<2.0), estamos bien. Si está entre 2 y 10, cuidado.
        is_struggling = current_loss > 2.0
        
        if is_weak and is_struggling:
            # Somos débiles y estamos fallando -> Protección
            return {
                'plasticity': torch.tensor(0.2, device=self.config.device),
                'memory': torch.tensor(0.9, device=self.config.device),
                'defense': torch.tensor(0.5, device=self.config.device),
                'supcon': torch.tensor(0.1, device=self.config.device),
                'symbiotic': torch.tensor(0.0, device=self.config.device),
                'sparsity': torch.tensor(0.0, device=self.config.device),
                'lr_scale': torch.tensor(0.5, device=self.config.device)
            }
        
        # --- PROCESAMIENTO NORMAL (FLOW) ---
        ...
        ], dtype=torch.float32, device=self.config.device).unsqueeze(0)
        
        encoded = self.state_encoder(state_vector)
        ...
        sparsity_gate = self.gate_sparsity(context).squeeze()
        
        # --- CONTROL DINÁMICO DE APRENDIZAJE ---
        base_lr_scale = self.gate_lr(context).squeeze()
        
        # Si el gradiente es muy alto, el orquestador debe ser cauteloso
        if grad_norm > 5.0:
            lr_scale = base_lr_scale * 0.5 # Reducir a la mitad
        else:
            # Rango normal [0.5, 1.5]
            lr_scale = base_lr_scale + 0.5 
            
        # Modulación por densidad (Modo Élite)
        if density < 0.10:
            # Si somos ligeros y no estamos en crisis (loss < 2.0), acelerar plasticidad
            plasticity = torch.clamp(plasticity + 0.2, 0.0, 1.0)
            # Anular presión de poda externa
            sparsity_gate = torch.tensor(0.0, device=self.config.device)
...
def get_dataloaders(config: Config):
    """DataLoaders con augmentation de alto rendimiento para CIFAR-10"""
    ...
    if config.dataset == 'CIFAR10':
        train_tf = transforms.Compose([
            ...
            # --- FIX: Augmentation fuerte para high-performance ---
            transforms.ColorJitter(brightness=0.2, contrast=0.2, saturation=0.2),  # Variación color
            transforms.ToTensor(),
            transforms.Normalize(mean, std),
            transforms.RandomErasing(p=0.2)  # Cutout regularization (occlusiones aleatorias)
            # --- FIN FIX ---
        ])
...
class ContinuumMemoryCell(nn.Module):
    def __init__(self, input_dim, hidden_dim, fast_lr=0.005, forget_rate=0.6, use_spectral=True):
        super().__init__()
    ...
    def forward(self, x, state_M=None, controls=None):
        ...
        state_summary = state_M_flat[:, :min(self.hidden_dim, state_M_flat.size(1))]
        ...
        if controls is not None:
            ...
            if torch.is_tensor(plasticity_factor): plasticity_factor = plasticity_factor.item()
            forget_gate = forget_gate * plasticity_factor
            update_gate = update_gate * plasticity_factor
        ...
        # --- FIX CRÍTICO: TANH CLAMP PARA ESTABILIDAD ---
        # Fuerza a que los pesos rápidos se mantengan acotados, evitando FastNorm > 200
        state_M_new = torch.tanh(state_M_new)
        # ------------------------------------------------
        ...
        if self.training and controls is not None:
            ...
            if torch.is_tensor(consolidation_factor): consolidation_factor = consolidation_factor.item()
            with torch.no_grad():
...
# =============================================================================
# BACKBONE CONVOLUCIONAL: CORTEZA VISUAL (V1-V4) [CORREGIDO]
# =============================================================================
...
class VisualCortex(nn.Module):
    def __init__(self, output_dim=128, grid_size=6):
        ...
        # --- FIX: Backbone ResNet-18 para capacidad visual profunda ---
        # Usamos torch.hub para la estructura pero sin pesos preentrenados (entrenamiento from scratch)
        resnet = torch.hub.load('pytorch/vision:v0.10.0', 'resnet18', pretrained=False)
        
        # Adaptación para CIFAR-10 (32x32):
        # Reemplazar la primera capa para no perder resolución espacial inicial
        self.conv1 = nn.Conv2d(3, 64, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn1 = resnet.bn1
        self.relu = resnet.relu
        # Eliminamos maxpool inicial para imágenes pequeñas
        self.layer1 = resnet.layer1
        self.layer2 = resnet.layer2
        self.layer3 = resnet.layer3
        self.layer4 = resnet.layer4
        
        # Proyección final para ajustar dimensiones al grid topológico
        # ResNet18 termina con 512 canales
        self.adapter = nn.Conv2d(512, output_dim, kernel_size=1)
        
        self.final_norm = nn.LayerNorm(output_dim)

    def forward(self, x):
        # Pipeline ResNet modificado
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        # No maxpool
        ...
        # Interpolación espacial para coincidir con el grid_size deseado (6x6)
        x = F.interpolate(x, size=(self.grid_size, self.grid_size), mode='bilinear', align_corners=False)
        
        # Adaptar canales a la dimensión del cerebro (512 -> 128)
...
class TopoBrainV24(nn.Module):
    def __init__(self, config: Config, in_channels: int = 3):
        ...
        print(f"\n🧬 TopoBrain v24.2 [HIGH-PERFORMANCE FIX]:")
        print(f"   Structure: ResNet18 Backbone + Cognitive Bypass")
        print(f"   Engine: Stabilized Nested Cells (Tanh Clamp)")
        
        # 1. Corteza Visual Potenciada
        self.visual_cortex = VisualCortex(output_dim=128, grid_size=config.grid_size)
        ...
        self.layer1 = AdaptiveCombinatorialComplexLayer(
            in_dim=128, hid_dim=128, num_nodes=num_nodes,
        ...
        # --- FIX: BYPASS COGNITIVO ---
        # Vía Cognitiva (Procesada por el cerebro)
        self.cognitive_readout = nn.Linear(256 * num_nodes, 512)
        
        # Vía Refleja (Directa de la visión, para robustez base)
        # La visual cortex saca 128 canales. Hacemos pooling global y proyectamos.
        self.reflex_readout = nn.Sequential(
            nn.Linear(128, 512),
            nn.LayerNorm(512),
            nn.ReLU()
        )
        
        # Clasificador final unificado
        self.final_mix = nn.Linear(512, 10)
        # -----------------------------
    ...
    def initialize_memories(self, dataloader):
        ...
        with torch.no_grad():
            for i, (x, y) in enumerate(dataloader):
                ...
                x_embed = features.flatten(2).transpose(1, 2)
    ...
    def _initialize_layer_memory(self, cell, x_input, name):
        if cell.memory_initialized == 1: return
    ...
    def consolidate_semantic_memories(self):
        if not self.config.use_nested_cells: return
        total_norm = 0.0
        num_cells = 0
        for layer_name, layer in [('layer1', self.layer1), ('layer2', self.layer2)]:
            ...
            if hasattr(layer, 'cell_mapper') and isinstance(layer.cell_mapper, ContinuumMemoryCell):
                ...
                total_norm += layer.cell_mapper.semantic_memory.norm().item()
                num_cells += 1
    ...
    def calculate_topology_diversity_loss(self, controls=None):
        if not self.config.use_plasticity: return torch.tensor(0.0, device=self.config.device)
        adj_w = torch.sigmoid(self.adj_weights)
        inc_w = torch.sigmoid(self.inc_weights)
        adj_entropy = -(adj_w * torch.log(adj_w + 1e-10) + (1 - adj_w) * torch.log(1 - adj_w + 1e-10)).mean()
        inc_entropy = -(inc_w * torch.log(inc_w + 1e-10) + (1 - inc_w) * torch.log(1 - inc_w + 1e-10)).mean()
        entropy_target = 0.69
        diversity_loss = ((adj_entropy - entropy_target) ** 2 + (inc_entropy - entropy_target) ** 2)
        if controls is not None:
    ...
    def forward(self, x, prev_states=None, controls=None):
        batch_size = x.size(0)
        with torch.amp.autocast(device_type='cuda', enabled=False):
            # 1. Extracción Visual (Backbone Potente)
            features = self.visual_cortex(x)  # [B, 128, 6, 6]
            
            # 2. Vía Refleja (Global Pooling para contexto general)
            # Permite que la red aprenda incluso si la topología está inestable
            reflex_features = features.mean(dim=[2, 3]) # [B, 128]
            reflex_out = self.reflex_readout(reflex_features) # [B, 512]
            
            # 3. Vía Cognitiva (Topológica)
            x_embed = features.flatten(2).transpose(1, 2) # [B, 36, 128]
            ...
            if prev_states is not None:
                ...
                for key, state in prev_states.items():
                    if state is not None and torch.is_tensor(state):
                        if torch.isnan(state).any() or torch.isinf(state).any() or state.size(0) != batch_size:
            ...
            # Procesamiento de salida cognitiva
            x_flat = x_l2.reshape(batch_size, -1)
            cognitive_out = self.cognitive_readout(x_flat) # [B, 512]
            
            # FUSIÓN DUAL: Reflejo + Cognitivo
            combined_features = reflex_out + cognitive_out
            logits = self.final_mix(combined_features)
            
            proj = None
            if self.proj_head and self.training:
                proj = self.proj_head(combined_features)
    ...
    def prune_topology(self, controls=None):
            if not self.config.use_adaptive_topology: return
            with torch.no_grad():
                adj_w = torch.sigmoid(self.adj_weights)
                ...
                if current_density < MIN_DENSITY:
                    total_elements = self.adj_mask.numel()
                    current_ones = self.adj_mask.sum().item()
                    target_ones = int(MIN_DENSITY * total_elements)
                    num_edges_needed = target_ones - current_ones
                    if num_edges_needed > 0:
                        print(f"  🌱 EMERGENCIA: Densidad {current_density:.4f} < {MIN_DENSITY}. Activando Neurogénesis...")
                        candidates_mask = ~self.adj_mask
                        zero_indices = candidates_mask.nonzero(as_tuple=False)
                        num_candidates = zero_indices.size(0)
                        if num_candidates > 0:
                            num_to_revive = min(num_edges_needed, num_candidates)
                            perm = torch.randperm(num_candidates, device=self.adj_mask.device)
                            idx_to_revive = zero_indices[perm[:num_to_revive]]
                            row_idx = idx_to_revive[:, 0]
                            col_idx = idx_to_revive[:, 1]
                            self.adj_mask[row_idx, col_idx] = True
                            self.adj_weights.data[row_idx, col_idx] = torch.abs(torch.randn(num_to_revive, device=self.adj_weights.device) * 0.1)
                    return
                ...
                proposed_mask = self.adj_mask & important_edges
                proposed_density = proposed_mask.float().mean().item()
                
                if proposed_density < MIN_DENSITY:
                    print(f"  🚫 ABORTADO: Poda resultaría en densidad {proposed_density:.4f} < {MIN_DENSITY:.4f}")
                    return
                
                self.adj_mask.data = proposed_mask
                ...
                if low_degree_nodes.any():
                    for i in torch.where(low_degree_nodes)[0]:
                        ...
                        if needed_connections > 0 and len(neighbors) > 0:
                            neighbor_tensor = torch.tensor(neighbors, device=adj_w.device)
                            neighbor_weights = adj_w[i, neighbor_tensor]
                            k_needed = min(needed_connections, len(neighbors))
                            _, top_k_idx = torch.topk(neighbor_weights, k_needed)
                            for idx in top_k_idx:
                                neighbor_idx = neighbors[idx]
                                self.adj_mask[i, neighbor_idx] = True
                                self.adj_mask[neighbor_idx, i] = True
                ...
                final_sparsity = 1.0 - self.adj_mask.float().mean().item()
...
def analyze_topology_flow(model, dataloader, run_name: str, num_samples=100):
    """Análisis de flujo de información con captura genérica de outputs"""
    if not model.config.use_plasticity:
        print("⚠️  Modelo no tiene topología aprendible")
        return
    ...
    with torch.no_grad():
        for i, (x, y) in enumerate(dataloader):
            ...
            # Captura genérica: tomar solo el primer elemento (output)
            # Los otros elementos (entropy, ortho, states) se ignoran
            l1_outputs = model.layer1(x_embed, curr_adj, curr_inc, adj_s, inc_s)
            if isinstance(l1_outputs, tuple):
                x_l1 = l1_outputs[0]
            else:
                x_l1 = l1_outputs
            ...
            l2_outputs = model.layer2(F.gelu(x_l1), curr_adj, curr_inc, adj_s, inc_s)
            if isinstance(l2_outputs, tuple):
                x_l2 = l2_outputs[0]
            else:
                x_l2 = l2_outputs
...
def make_adversarial_pgd(model, x, y, eps, steps, dataset_name, controls=None, prev_states=None):
    """
    PGD ataque con congelamiento total de pesos y detach explícito de estados.
    FIX: Asegura que el grafo computacional no se rompa y que los estados previos sean genuinamente independientes.
    """
    was_training = model.training
    model.eval()
    
    # Captura de estados previos con detach absoluto
    safe_controls = None
    if controls is not None:
        safe_controls = {}
    ...
    # Guardar requires_grad original
    prev_grad_states = {}
    ...
    try:
        # Inicialización del ruido adversarial
        delta = torch.zeros_like(x).uniform_(-eps, eps)
        delta = delta.detach().requires_grad_(True)
        actual_steps = steps
        
        # Modulación de pasos por defensa
        if safe_controls is not None and 'defense' in safe_controls:
            defense_val = safe_controls['defense']
            if torch.is_tensor(defense_val): defense_val = defense_val.item()
            actual_steps = max(3, int(steps * defense_val))
        
        alpha = eps / max(1, actual_steps) * 2.0
        
        for step in range(actual_steps):
            # Forward con estados previos DETACH completamente
            detached_prev_states = None
            if prev_states is not None:
                detached_prev_states = {k: v.detach() for k, v in prev_states.items() if v is not None}
            
            logits, _, _, _, _ = model(x + delta, detached_prev_states, safe_controls)
            ...
            # Actualización del delta con gradiente normalizado
            with torch.no_grad():
                if delta.grad is not None:
                    grad = delta.grad
...
def evaluate(model, loader, config, adversarial=False, controls=None):
    """
    Evaluación con plasticidad residual (test-time adaptation)
    ...
    # Configurar controles para evaluación
    eval_controls = None
    if controls is not None:
        eval_controls = {k: (v.detach() if torch.is_tensor(v) else v) for k, v in controls.items()}
    else:
        # Controles por defecto: plasticidad mínima, sin regularizaciones
        eval_controls = {
            'plasticity': torch.tensor(0.05, device=config.device),  # 5% plasticidad residual para TTA
            'memory': torch.tensor(0.8, device=config.device),
            'defense': torch.tensor(0.2, device=config.device),
            'supcon': torch.tensor(0.0, device=config.device),
            'symbiotic': torch.tensor(0.0, device=config.device),
            'sparsity': torch.tensor(0.0, device=config.device),
            'lr_scale': torch.tensor(1.0, device=config.device)
        }
    
    # Congelar pesos slow, permitir actualización leve de fast weights
    slow_params = []
    for name, param in model.named_parameters():
        # Solo congelamos lo estructural, dejamos libre lo dinámico si es necesario
        if 'W_slow' in name or 'adj_weights' in name or 'inc_weights' in name or 'orchestrator' in name:
            slow_params.append((name, param.requires_grad))
    ...
    try:
        for i, (x, y) in enumerate(loader):
            ...
            # Micro-adaptación: permitir que fast weights capturen brevemente el input
            # No usamos torch.no_grad() globalmente para permitir TTA si se implementa
            with torch.no_grad():
        ...
        # Restaurar estado de gradientes
        for name, requires_grad in slow_params:
...
# 6. TRAINING LOOP 
# =============================================================================
def train_epoch(model, loader, optimizer, opt_topo, config, epoch, monitor, scaler, sparsity_lambda):
    """Entrenamiento homeostático con lista manual en lugar de deque"""
    model.train()
    
    metrics = {
        'loss': 0.0, 'acc': 0.0, 'supcon_loss': 0.0, 'ortho_loss': 0.0, 
        'topology_diversity': 0.0, 'sparsity_loss': 0.0,
        'plasticity_gate': 0.0, 'defense_gate': 0.0, 'memory_gate': 0.0, 'supcon_gate': 0.0,
        'symbiotic_gate': 0.0, 'sparsity_gate': 0.0, 'lr_scale_gate': 0.0,
        'running_loss': 0.5, 'prev_loss': float('inf'), 'delta_loss': 0.0,
        'explosion_streak': 0, 'grad_norm': 0.0
    }
    
    supcon = SupConLoss() if config.use_supcon else None
    topology_warmup = epoch > config.topo_warmup_epochs
    
    # Lista manual en lugar de deque
    grad_norm_window = []
    explosion_streak = 0
    
    controls = None
    total_norm = torch.tensor(0.0, device=config.device, dtype=torch.float32)
    ...
    for i, (x, y) in enumerate(loader):
        try:
            ...
            if not torch.isfinite(x).all() or not torch.isfinite(y).all():
                print(f"⚠️  Input NaN/Inf detectado en batch {i}, skip")
                continue
            
            with torch.no_grad():
                current_loss_val = metrics['running_loss']
                delta_loss = current_loss_val - metrics['prev_loss']
                metrics['prev_loss'] = current_loss_val
                
                if hasattr(model, 'adj_weights') and model.adj_weights is not None:
                    adj_w_current = torch.sigmoid(model.adj_weights)
                    current_density = (adj_w_current > 0.5).float().mean().item()
                else:
                    current_density = 1.0
                
                sparsity = 1.0 - current_density
                grad_norm_val = np.mean(grad_norm_window[-10:]) if grad_norm_window else 1.0  # Últimos 10
                
                metrics_dict = {
                    'loss': min(current_loss_val, 1000.0),
                    'grad_norm': min(grad_norm_val, 100.0),
                    'sparsity': sparsity,
                    'entropy': min(metrics.get('entropy', 0.5), 10.0),
                    'batch_var': float(x.var().clamp(0.0, 10.0).item()),
                    'epoch_progress': min(epoch / config.epochs, 1.0),
                    'memory_norm': min(metrics.get('memory_norm', 1.0), 10.0),
                    'L_score': min(metrics.get('L_score', 2.0), 100.0),
            ...
            if model.orchestrator is not None and config.use_orchestrator:
                try:
                    ...
                    for k in ['plasticity', 'defense', 'memory', 'supcon', 'symbiotic', 'sparsity', 'lr_scale']:
                        if k not in controls or controls[k] is None:
                            controls[k] = torch.tensor(0.5, device=config.device)
                        if torch.isnan(controls[k]) or torch.isinf(controls[k]):
                            controls[k] = torch.tensor(0.5, device=config.device)
                    ...
                    print(f"⚠️  Orquestador falló: {e}, usando controles por defecto")
                    controls = {k: torch.tensor(0.5, device=config.device) for k in ['plasticity', 'defense', 'memory', 'supcon', 'symbiotic', 'sparsity', 'lr_scale']}
            
            if current_loss_val > 200.0:
                explosion_streak += 1
            else:
                explosion_streak = max(0, explosion_streak - 1)
            
            if explosion_streak >= 2:
                print(f"  🚨 CRISIS: Loss {current_loss_val:.1f} por {explosion_streak} batches. SKIP.")
                optimizer.zero_grad()
                if opt_topo: opt_topo.zero_grad()
                continue
            ...
            if not torch.isfinite(x_adv).all():
                print(f"⚠️  Adversarial NaN/Inf en batch {i}, usando original")
                x_adv = x
            
            try:
                logits, proj, entropy, ortho_deviation, new_states = model(x_adv, detached_states, controls)
            except Exception as e:
                print(f"💥 Forward falló en batch {i}: {e}, skip")
                continue
            
            if not torch.isfinite(logits).all():
                print(f"⚠️  Logits NaN/Inf en batch {i}, skip")
                continue
            
            loss = F.cross_entropy(logits, y)
            
            if config.use_supcon and supcon is not None and proj is not None:
                if torch.isfinite(proj).all():
            ...
            try:
                ...
                print(f"💥 Backward falló en batch {i}: {e}")
                optimizer.zero_grad()
                if opt_topo: opt_topo.zero_grad()
                continue
            ...
            if (i + 1) % config.accumulation_steps == 0:
                ...
                grad_norm_window.append(min(total_norm.item(), 100.0))
                
                # Mantener solo últimos 10 valores
                if len(grad_norm_window) > 10:
                    grad_norm_window.pop(0)
                
                if not torch.isfinite(total_norm):
                    print(f"  💥 GRADIENTE INF/NAN. Reset.")
                    optimizer.zero_grad()
                    if opt_topo: opt_topo.zero_grad()
                    continue
                ...
                if controls is not None and 'lr_scale' in controls:
                    ...
                    lr_scale = max(0.1, min(lr_scale, 1.0))
            ...
            if new_states is not None:
                detached_states = {k: v.detach() if torch.is_tensor(v) and v is not None else None 
                                 for k, v in new_states.items()}
            if i % 100 == 0:
                detached_states = None
            
            metrics['loss'] += loss.item() * config.accumulation_steps
            metrics['acc'] += float(logits.argmax(1).eq(y).sum().item())
            ...
            if i % 50 == 0:
                ...
                if controls is not None:
                    status = "🚨CRISIS" if explosion_streak > 0 else "  NORMAL"
                    print(f"    🧠 [Prefrontal | Batch {i:03d}] {status} "
                          f"Plast:{controls['plasticity'].item():.2f} | "
                          f"Def:{controls['defense'].item():.2f} | "
                          f"Mem:{controls['memory'].item():.2f} | "
                          f"LR:{controls['lr_scale'].item():.2f} | "
                          f"Grad:{total_norm.item():.1f}")
        
        except Exception as e:
            print(f"⚠️  Batch {i} falló: {e}, continuando")
            optimizer.zero_grad()
            if opt_topo: opt_topo.zero_grad()
            continue
    
    n = max(len(loader), 1)
    d = max(len(loader.dataset), 1)
    
    metrics['loss'] /= n
    metrics['acc'] = 100 * metrics['acc'] / d
    
    # Añadir densidad
    metrics['explosion_streak'] = explosion_streak
    if config.use_plasticity and hasattr(model, 'adj_weights') and model.adj_weights is not None:
        with torch.no_grad():
            adj_w = torch.sigmoid(model.adj_weights)
            metrics['density'] = (adj_w > 0.5).float().mean().item() if adj_w.numel() > 0 else 0.0
...
def train_model(config: Config, run_name: str):
    ...
    # --- OPTIMIZADORES V24.2 HIGH-PERFORMANCE ---
    main_params = [p for n, p in model.named_parameters() if 'weights' not in n and 'importance' not in n and 'orchestrator' not in n and 'visual_cortex' not in n and p.requires_grad]
    visual_params = [p for n, p in model.named_parameters() if 'visual_cortex' in n and p.requires_grad]
    ...
    # Visual Cortex (ResNet18): LR bajo para preservar features pre-aprendidos o aprender establemente
    optimizer_visual = optim.AdamW(visual_params, lr=config.lr_main * 0.1, weight_decay=1e-3)
    
    # Main Brain (Linear Layers): LR medio
    optimizer_main = optim.AdamW(main_params, lr=config.lr_main, weight_decay=config.weight_decay)
    
    # Schedulers
    scheduler_main = optim.lr_scheduler.MultiStepLR(optimizer_main, milestones=[30, 60, 90, 120], gamma=0.2)
    scheduler_visual = optim.lr_scheduler.MultiStepLR(optimizer_visual, milestones=[30, 60, 90, 120], gamma=0.2)
    
    opt_topo = None
    sched_topo = None
    if config.use_plasticity and topo_params:
        opt_topo = optim.AdamW(topo_params, lr=config.lr_topo, weight_decay=0.0)
        def warmup_topo(epoch):
            return min(1.0, max(0.1, epoch / config.topo_warmup_epochs))
    ...
    print(f"LR_Main: {config.lr_main} | LR_Visual: {config.lr_main * 0.1}")
    print(f"{'='*60}\n")
    ...
    for epoch in range(1, config.epochs + 1):
        ...
        try:
            train_metrics = train_epoch(
                model, train_loader, optimizer_main, opt_topo, 
                config, epoch, monitor=topo_monitor, scaler=scaler, 
                sparsity_lambda=current_sparsity_lambda
            )
            
            if config.use_nested_cells and visual_params:
        ...
        status = "🚨CRISIS" if train_metrics.get('explosion_streak', 0) > 0 else "  NORMAL"
        log_msg = (f"Epoch {epoch:02d}/{config.epochs} {status} | "
                  f"Loss: {train_metrics['loss']:.4f} | "
                  f"Acc: {train_metrics['acc']:.2f}% | "
                  f"SupCon: {train_metrics.get('supcon_loss', 0):.4f} | "
                  f"Ortho: {train_metrics.get('ortho_loss', 0):.4f} | "
                  f"LR_M: {optimizer_main.param_groups[0]['lr']:.6f} | "
                  f"LR_V: {optimizer_visual.param_groups[0]['lr']:.6f}")
        
        if config.use_orchestrator:
            log_msg += (f" | Gates: P:{train_metrics['plasticity_gate']:.2f} "
                       f"D:{train_metrics['defense_gate']:.2f} "
                       f"M:{train_metrics['memory_gate']:.2f}")
        ...
        if train_metrics['acc'] > best_acc:
            ...
            checkpoint_data = {
                ...
                'optimizer_visual_state': optimizer_visual.state_dict(),
