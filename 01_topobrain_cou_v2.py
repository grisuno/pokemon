...
# =============================================================================
# MATRIZ DE ABLATION CIENTÍFICO (11 experimentos para CPU)
# =============================================================================
ABLATION_MATRIX = [
    ("Baseline", {}),
    ...
    ("Plasticity_Nested", {"use_plasticity": True, "use_nested_cells": True}),
    ("Plasticity_Symbiotic", {"use_plasticity": True, "use_symbiotic": True}),
    ("Nested_SupCon", {"use_nested_cells": True, "use_supcon": True}),
    ("Full_But_Plasicity", {
...
def run_scientific_ablation():
    ...
    # Ordenar por robustez (PGD accuracy) para análisis pero mostrar todos
    ...
    print("\n📋 RESULTADOS COMPLETOS DE ABLATION (Ordenados por PGD Accuracy):")
    print("-"*80)
    print(f"{'Rank':<6} {'Experimento':<30} {'Clean Acc':<10} {'PGD Acc':<10} {'Gap':<8} {'Estabilidad':<12}")
    print("-"*80)
    
    for i, result in enumerate(sorted_results, 1):
        summary = result['summary']
        print(f"{i:<6} {result['name']:<30} "
              f"{summary['final_clean_acc']:>8.2f}%  "
              f"{summary['final_pgd_acc']:>8.2f}%  "
              f"{summary['robustness_gap']:>6.2f}%  "
              f"{summary['stability_score']:>10.3f}")
    ...
    print("-"*60)
    all_clean_accs = [r['summary']['final_clean_acc'] for r in all_results]
    all_pgd_accs = [r['summary']['final_pgd_acc'] for r in all_results]
    all_gaps = [r['summary']['robustness_gap'] for r in all_results]
    all_stabilities = [r['summary']['stability_score'] for r in all_results]
    
    print(f"Clean Accuracy:  Media={np.mean(all_clean_accs):.2f}%, Std={np.std(all_clean_accs):.2f}%")
    print(f"PGD Accuracy:    Media={np.mean(all_pgd_accs):.2f}%, Std={np.std(all_pgd_accs):.2f}%")
    print(f"Robustness Gap:  Media={np.mean(all_gaps):.2f}%, Std={np.std(all_gaps):.2f}%")
    print(f"Stability Score: Media={np.mean(all_stabilities):.3f}, Std={np.std(all_stabilities):.3f}")
    
    # Análisis de contribución de mecanismos
    print("\n🔬 ANÁLISIS DE CONTRIBUCIÓN DE MECANISMOS:")
    print("-"*60)
    
    mechanisms = ['plasticity', 'nested_cells', 'mgf', 'supcon', 'symbiotic']
    ...
    if baseline:
        ...
        for mech in mechanisms:
            # Encontrar experimentos con este mecanismo
            with_mech = [r for r in all_results if mech.replace('_', '') in r['name'].lower()]
            
            if with_mech:
                avg_pgd = sum(r['summary']['final_pgd_acc'] for r in with_mech) / len(with_mech)
                improvement = avg_pgd - baseline_pgd
                print(f"• {mech.replace('_', ' ').title():<15}: "
                      f"PGD improvement = {improvement:+5.2f}% "
                      f"(Baseline: {baseline_pgd:.2f}% → Avg: {avg_pgd:.2f}%)")
    
    # Análisis de sinergias
    print("\n🤝 ANÁLISIS DE SINERGIAS:")
    print("-"*60)
    
    pair_results = [r for r in all_results if '_' in r['name'] and r['name'] not in ['Baseline', 'Full_Topobrain']]
    if pair_results:
        best_pair = max(pair_results, key=lambda x: x['summary']['final_pgd_acc'])
        print(f"Mejor combinación de dos mecanismos: {best_pair['name']}")
        print(f"  PGD accuracy: {best_pair['summary']['final_pgd_acc']:.2f}%")
        print(f"  Mejora vs baseline: {best_pair['summary']['final_pgd_acc'] - baseline_pgd:.2f}%")
    
    # Identificar experimentos con gap negativo (PGD > Clean)
    print("\n⚠️  EXPERIMENTOS CON ADVERSARIAL POSITIVE GAP (PGD > Clean):")
    print("-"*60)
    negative_gaps = [r for r in all_results if r['summary']['robustness_gap'] < 0]
    if negative_gaps:
        for r in negative_gaps:
            summary = r['summary']
            print(f"• {r['name']:<30}: PGD={summary['final_pgd_acc']:.2f}% > Clean={summary['final_clean_acc']:.2f}%")
    else:
        print("  Ningún experimento mostró PGD > Clean (gap negativo).")
