...
# CPU-friendly, sin GPU, con estudio de ablación controlado
# =============================================================================
import torch
...
@dataclass
class Config:
    # ABLATION FLAGS - componentes alineados con NL
...
# ---------------------------
# CMS LAYER - Implementación de Nested Learning
# ---------------------------
...
class CMSLayer(nn.Module):
    def __init__(self, dim: int, config: Config):
        ...
        # Pesos lentos (entrenables por backprop)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # Forward lento (gradientes fluyen aquí)
        slow_out = F.linear(x, self.slow_weight, self.slow_bias)

        if self.training:
            # --- Nivel 1: Delta Rule (alta frecuencia) ---
            if self.config.USE_CMS_LEVEL1:
                grad = slow_out.detach()  # surrogate for ∇_y L
                if self.config.LEARNING_RULE == "dgd":
                    # DGD: W ← W(I - η xx^T) - η ∇L ⊗ x
                    outer = torch.bmm(x.unsqueeze(2), x.unsqueeze(1)).mean(dim=0)  # E[xx^T]
...
def train(config: Config):
    logger.info("🧠 NESTED CMS CIFAR - POC basada en Nested Learning (Google)")
    logger.info("=" * 80)
    ...
    torch.save({
        ...
        'config': vars(config),
        'final_acc': eval_metrics['accuracy']
    }, "nested_cms_cifar.pth")
