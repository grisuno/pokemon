"""
================================================================================
NeuroLogos v5.1-CPU - Ablación Científica Rigurosa (Vision-Language)
================================================================================
Matriz de ablación: 11 experimentos | Validación cruzada | Análisis estadístico
Componentes: GRID (G) | SYMBIOTIC (S) | ADVERSARIAL (A)
CPU-OPTIMIZADO: batch=16, epochs=15, cv_folds=2, num_workers=0
================================================================================
"""
...
# ============================================================================
# UTILIDADES CIENTÍFICAS (FIX #1 y #2)
# ============================================================================
...
# ============================================================================
# CONFIGURACIÓN CIENTÍFICA CPU-OPTIMIZADA (FIX #3)
# ============================================================================
@dataclass
class NeuroLogosConfig:
    """Configuración CPU-friendly para NeuroLogos"""
    device: str = "cpu"
    seed: int = 42
    ...
    # Hiperparámetros reducidos para CPU
    batch_size: int = 16
    epochs: int = 15
    lr: float = 0.001
    weight_decay: float = 1e-5
    
    # Parámetros adversariales (más ligeros)
    adv_epsilon: float = 0.05
    adv_alpha: float = 0.005
    adv_steps: int = 3
    adv_start_epoch: int = 3
    
    # Regularización
    lambda_density: float = 0.01
    
    # Estabilidad
    clip_value: float = 1.0
    
    # Validación cruzada (menos folds para CPU)
    cv_folds: int = 2
...
def run_scientific_ablation():
    ...
    print("="*80)
    print("🧠 NeuroLogos v5.1-CPU - Ablación Científica Rigurosa")
    print("="*80)
    print(f"📊 11 experimentos | 2 folds CV | Stats: t-test, Cohen's d, CI95")
    print(f"⚙️ Componentes: GRID, SYMBIOTIC, ADVERSARIAL")
    print(f"💻 CPU-OPTIMIZADO: batch=16, epochs=15, cv_folds=2, num_workers=0")
    print("="*80 + "\n")
