...
class TopologicalHealthSovereignty:
    ...
    def _analyze_matrix(self, weight_matrix: torch.Tensor, name: str) -> TopologyMetrics:
            """Análisis SVD de matriz topológica (CORREGIDO)"""
            ...
            # --- FIX CRÍTICO: Definición correcta de Sparsity ---
            # Sparsity = Porcentaje de la matriz que es CERO (o muy bajo)
            density = (W > 0.5).mean() # Esto es qué tanto está llena
            sparsity = 1.0 - density   # Esto es qué tanto está vacía
            mean_weight = W.mean()
            
            # Diagnóstico ajustado a la nueva definición
            status = "CRITICAL"
            action = ""
            
            if sparsity > 0.99: # 99% vacío -> Muerto
                status = "PRUNED/DEAD"
                action = f"Aumenta conectividad."
            elif sparsity < 0.10: # Menos de 10% vacío -> Bloque de cemento
                status = "COLLAPSED (TOO DENSE)"
                action = f"Red saturada. Aumentar lambda_sparsity."
            elif L_score < 2.0:
                status = "DEGENERATE"
                action = "Estructura aleatoria. Revisar init."
            elif sparsity > 0.40 and L_score > 4.0:
                status = "SOBERANO"
                action = "Óptimo."
            else:
                status = "EMERGENTE"
...
class AdaptiveCombinatorialComplexLayer(nn.Module):
    ...
    def __init__(self, in_dim, hid_dim, num_nodes, config: Config, 
        ...
        super().__init__()
        print(f"DEBUG Layer {layer_idx}: num_nodes={num_nodes}") 
...
class TopoBrainNetV18(nn.Module):
    ...
    def __init__(self, config: Config, in_channels: int = 3):
        ...
        # --- FIX: Cálculo dinámico del Patch Size ---
        # Asumimos 32x32 para CIFAR10. Si usas MNIST (28x28) el ajuste sería automático
        # pero para grid exactos se recomienda que img_size sea divisible por grid_size.
        img_size = 28 if config.dataset == 'MNIST' else 32
        
        # El patch_size asegura que la salida del embedding tenga exactamente grid x grid tokens
        patch_size = img_size // grid
        if patch_size < 1: patch_size = 1 # Evitar kernel 0
        
        print(f"DEBUG: Grid={grid}x{grid} | Img={img_size}x{img_size} | Patch Size calculado={patch_size}")
        ...
        # Capas combinatoriales con num_nodes
    ...
    def _init_grid_topology(self, N):
        ...
        # Celdas 2D
        cells = []
        # Fix: Manejo de grid muy pequeños para evitar error de rango vacío
        if N > 1:
            for r in range(N - 1):
                for c in range(N - 1):
                    tl = r * N + c
            ...
            # Caso degenerado (grid 1x1), no hay celdas, incidencia vacía
            pass
        
        num_cells = len(cells)
        # Fix: Si no hay celdas, crear tensor dummy para evitar crash en backward
        if num_cells == 0:
            inc = torch.zeros(num_nodes, 1) 
        else:
    ...
    def get_topology(self, return_sparse=False):
            ...
            # 1. Adjacency con máscara y sigmoide
            ...
            degree = adj_sparse_mask.sum(dim=1, keepdim=True).clamp(min=1e-6)
            curr_adj = adj_sparse_mask / degree
            
            # 2. Incidence normalizada
    ...
    def calculate_ortho_loss(self):
        ...
        loss = 0
        layers_data = [(self.layer1, 0), (self.layer2, 1)]
    ...
    def prune_topology(self):
        ...
        with torch.no_grad():
            ...
            # Forzar conectividad mínima
            if hasattr(self, 'config'):
                min_connections = max(1, self.config.grid_size // 2)
...
def train_model(config: Config, run_name: str):
    """Loop de entrenamiento v18 (CORREGIDO - Inicialización Negativa)"""
    # Setup
    ...
    # --- FIX CRÍTICO: Inicialización para Sparsity ---
    if config.use_plasticity:
        with torch.no_grad():
            # 1. Inicializar todo en NEGATIVO (Silencio)
            # sigmoid(-3.0) es aprox 0.05. La red empieza vacía.
            model.adj_weights.data.normal_(-3.0, 1.0)
            model.inc_weights.data.normal_(-3.0, 1.0)
            
            # 2. Activar SOLO conexiones necesarias (Grid base)
            grid = config.grid_size
            count_active = 0
            for i in range(grid * grid):
                ...
                for offset in [-grid, grid, -1, 1]:
                    j = i + offset
                    if 0 <= j < grid * grid:
                        # Verificar bordes para no saltar de línea erróneamente
                        if abs((i // grid) - (j // grid)) + abs((i % grid) - (j % grid)) == 1:
                            # Valor positivo fuerte para activar estas conexiones específicas
                            model.adj_weights.data[i, j] = 2.0 
                            count_active += 1
            print(f"  ⚡ Topología inicializada: {count_active} conexiones activas (Resto silenciadas).")
    # -------------------------------------------------
    ...
    if config.use_plasticity and topo_params:
        ...
        # Warmup simple para la topología
    ...
    for epoch in range(1, config.epochs + 1):
        ...
        if config.use_plasticity:
            # Calcular densidad REAL para mostrar en consola
            density = (torch.sigmoid(model.adj_weights) > 0.5).float().mean().item()
            log_msg += f" | Densidad: {density:.2%} (Sparsity: {1-density:.2%})"
