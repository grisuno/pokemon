#%%writefile neurosoberano_bicameral_clean.py
# =============================================================================
# NeuroLogos Bicameral Minimalista v1.0
# Dataset: Flickr8k (30K images, 1GB - perfecto para Colab gratuito)
# =============================================================================
import torch
...
IMG_SIZE = 224
MAX_CAPTION_LEN = 30
VOCAB_SIZE = 5000  # Reducido para dataset más pequeño
EMBED_DIM = 256
HIDDEN_DIM = 512
...
def setup_flickr8k(data_dir='./data'):
    """
    Descarga Flickr8k automáticamente desde Kaggle
    ...
    print("✅ Flickr8k listo para usar\n")
    return flickr_dir
...
# =============================================================================
# HEMISFERIO DERECHO: Procesamiento Visual (ResNet pretrained)
# =============================================================================
class RightHemisphere(nn.Module):
    ...
    def forward(self, image, plasticity=0.1, transfer_rate=0.005):
        ...
        return visual_thought  # [B, 512]

# =============================================================================
# HEMISFERIO IZQUIERDO: Procesamiento de Lenguaje
# =============================================================================
class LeftHemisphere(nn.Module):
    ...
    def forward(self, visual_context, captions=None, max_len=30, return_gate=False, temperature=0.9):
...
class CorpusCallosum(nn.Module):
    ...
    def forward(self, right_features):
        ...
        return alpha * transformed + (1 - alpha) * right_features
...
class NeuroLogosBicameral(nn.Module):
    ...
    def forward(self, image, captions=None, plasticity=0.1, transfer_rate=0.005, return_diagnostics=False, temperature=0.9):
        ...
        if return_diagnostics and captions is not None:
            output, gate = self.left_hemisphere(visual_context, captions, return_gate=True, temperature=temperature)
            return output, visual_features, visual_context, gate
        
        output = self.left_hemisphere(visual_context, captions, temperature=temperature)
        return output


class NeuralDiagnostics:
    """Sistema de monitoreo de salud cerebral bicameral"""
    def __init__(self):
        self.history = {
            'loss': [],
            'right_liquid_norm': [],
            'callosal_flow': [],
            'left_gate_activation': [],
            'vocab_diversity': [],
            'plasticity_effective': []
        }
    
    def measure_callosal_flow(self, right_features, left_context):
        """Mide qué tan bien está fluyendo información entre hemisferios"""
        with torch.no_grad():
    ...
    def measure_vocab_diversity(self, generated_tokens, vocab_size):
        """Mide diversidad de vocabulario generado (evitar colapso)"""
        unique_tokens = len(torch.unique(generated_tokens))
        diversity = unique_tokens / vocab_size
        return diversity
    
    def measure_gate_health(self, gate_activations):
        """Verifica que el liquid gate no colapse a 0 o 1"""
        with torch.no_grad():
            mean_gate = gate_activations.mean().item()
            std_gate = gate_activations.std().item()
            # Saludable: mean ~0.5, std > 0.1
            health_score = 1.0 - abs(mean_gate - 0.5) - max(0, 0.15 - std_gate)
            return mean_gate, std_gate, health_score
    ...
    def report(self, epoch):
        ...
        # Right Hemisphere (Liquid Neuron Health)
        if len(self.history['right_liquid_norm']) > 0:
            avg_liquid = np.mean(self.history['right_liquid_norm'][-recent:])
            status = "🟢 Estable" if 0.5 < avg_liquid < 2.5 else "🔴 Inestable"
            print(f"👁️  Right Hemisphere Liquid Norm: {avg_liquid:.3f} {status}")
        
        # Corpus Callosum (Comunicación Interhemisférica)
        if len(self.history['callosal_flow']) > 0:
            avg_flow = np.mean(self.history['callosal_flow'][-recent:])
            status = "🟢 Fluido" if avg_flow > 0.3 else "🟡 Débil" if avg_flow > 0.1 else "🔴 Bloqueado"
            print(f"🔗 Corpus Callosum Flow: {avg_flow:.3f} {status}")
        
        # Left Hemisphere (Gate Health)
        if len(self.history['left_gate_activation']) > 0:
            avg_gate = np.mean(self.history['left_gate_activation'][-recent:])
            status = "🟢 Modulando" if 0.3 < avg_gate < 0.7 else "🟡 Sesgado"
            print(f"💬 Left Hemisphere Gate: {avg_gate:.3f} {status}")
        
        # Vocabulario (Evitar Colapso)
        if len(self.history['vocab_diversity']) > 0:
            avg_div = np.mean(self.history['vocab_diversity'][-recent:])
            status = "🟢 Diverso" if avg_div > 0.1 else "🟡 Limitado" if avg_div > 0.05 else "🔴 Colapsado"
            print(f"📚 Vocab Diversity: {avg_div:.3f} {status}")
        
        # Plasticidad
        if len(self.history['plasticity_effective']) > 0:
            avg_plast = np.mean(self.history['plasticity_effective'][-recent:])
            print(f"🧬 Plasticidad Efectiva: {avg_plast:.4f}")
...
def train_bicameral():
    ...
    for epoch in range(start_epoch, 30):
        ...
        temperature = max(0.7, 1.2 - epoch * 0.02)
        ...
        for batch_idx, (images, captions) in enumerate(pbar):
            ...
            gate_loss = -gate_entropy * 0.01
            
            probs = F.softmax(logits, dim=-1)
            vocab_entropy = -(probs * torch.log(probs + 1e-8)).sum(dim=-1).mean()
            vocab_loss = -vocab_entropy * 0.005
            
            loss = ce_loss + gate_loss + vocab_loss
            
            with torch.no_grad():
                model.right_hemisphere.spatial_liquid.prediction_error = \
                    (ce_loss / 8.0).clamp(0.0, 0.8).item()
            ...
            with torch.no_grad():
                ...
                vocab_div = None
                if batch_idx % 50 == 0:
                    sample_gen = model(images[:4], captions=None, temperature=temperature)
            ...
            pbar.set_postfix({
                ...
                'gate': f'{gate_mean:.2f}±{gate_std:.2f}'
            })
        ...
        if epoch % 2 == 0:
            ...
            with torch.no_grad():
                for sample_idx in range(3):
                    ...
                    generated = model(sample_img, captions=None, temperature=0.8)
