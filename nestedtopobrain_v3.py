...
class PrefrontalOrchestrator(nn.Module):
    ...
    def forward(self, metrics_dict: Dict[str, float]) -> Dict[str, torch.Tensor]:
        """
        Orquestador v26: Allostasis (Adaptación Predictiva Valiente).
        ...
        """
        # 1. Extraer métricas críticas para la supervivencia
        density = metrics_dict.get('density', 0.1)
        current_loss = metrics_dict.get('loss', 10.0)
        
        # 2. LÓGICA DE SUPERVIVENCIA CONDICIONAL
        
        # Definición de Estado Crítico:
        # Somos frágiles (baja densidad) Y estamos fallando (loss alto).
        # Nota: El umbral de loss 2.0 es una barrera de seguridad; si sube de ahí, hay problemas.
        is_weak = density < 0.05
        is_failing = current_loss > 2.0 
        
        if is_weak and is_failing:
            # --- MODO PÁNICO (Solo si es estrictamente necesario) ---
            return {
                'plasticity': torch.tensor(0.1, device=self.config.device), # Frenar cambios
                'memory': torch.tensor(0.95, device=self.config.device),    # Aferrarse a lo conocido
                'defense': torch.tensor(0.8, device=self.config.device),    # Escudos al máximo
                'supcon': torch.tensor(0.5, device=self.config.device),     # Buscar estructuras simples
                'symbiotic': torch.tensor(0.0, device=self.config.device),  # Apagar refinamiento complejo
                'sparsity': torch.tensor(0.0, device=self.config.device),   # DETENER PODA INMEDIATAMENTE
                'lr_scale': torch.tensor(0.5, device=self.config.device)    # Reducir velocidad de aprendizaje
            }
            
        # 3. PROCESAMIENTO NORMAL / MODO ÉLITE
        
        # Construir vector de estado para la RNN
        ...
        # Codificación y Contexto Temporal
        ...
        # Generar señales base neuronales
        ...
        lr_scale = self.gate_lr(context).squeeze() * 1.9 + 0.1

        # 4. MODULACIÓN DE "FLUJO" (FLOW STATE)
        # Si la densidad es baja pero el loss es aceptable, estamos en "Modo Élite".
        # No debemos frenar, debemos acelerar la adaptación.
        
        if density < 0.10:
            # Boost de Plasticidad: "Eres ligero, usa tu velocidad"
            # Forzamos que la plasticidad suba para aprovechar los Fast Weights
            plasticity = torch.clamp(plasticity + 0.2, 0.0, 1.0)
            
            # Defensa Moderada: Mantener alerta pero no cerrarse
            defense = torch.clamp(defense, 0.2, 0.6)
            
            # Protección de Estructura:
            # Si la densidad baja del 6%, anulamos la presión de sparsity del orquestador
            # para que el mecanismo de poda natural no se sienta forzado a cortar más.
            if density < 0.06:
                sparsity_gate = torch.tensor(0.0, device=self.config.device)

        # Empaquetar controles finales
        controls = {
...
class TopoBrainV24(nn.Module):
    ...
    def prune_topology(self, controls=None):
            """Poda topológica con protocolo de supervivencia garantizado y neurogénesis"""
            ...
            with torch.no_grad():
                ...
                # --- PARTE 1: NEUROGÉNESIS DE EMERGENCIA (SUELO DE DENSIDAD) ---
                if current_density < MIN_DENSITY:
                    ...
                    if num_edges_needed > 0:
                        ...
                        # Encontrar candidatos (donde NO hay conexión)
                        ...
                        if num_candidates > 0:
                            ...
                            # 1. Revivir la máscara
                            self.adj_mask[row_idx, col_idx] = True
                            
                            # 2. Reiniciar el peso (Chispazo de vida)
                            # Usamos valor absoluto pequeño para que empiece positivo pero débil
                            self.adj_weights.data[row_idx, col_idx] = torch.abs(torch.randn(num_to_revive, device=self.adj_weights.device) * 0.1)
                            
                            new_density = self.adj_mask.float().mean().item()
                            print(f"     ✅ Neurogénesis completada: {num_to_revive} sinapsis creadas. Nueva densidad: {new_density:.4f}")
                    
                    # Si estamos en emergencia, NO podar en este ciclo
                    return

                # --- PARTE 2: LÓGICA DE PODA STANDARD (Solo si estamos seguros) ---
                ...
                # Garantizar conectividad mínima local (Grid)
...
def train_epoch(model, loader, optimizer, opt_topo, config, epoch, monitor, scaler=None, sparsity_lambda=None):
    """Entrenamiento homeostático con inicialización de estados y gestión de densidad"""
    model.train()
    ...
    for i, (x, y) in enumerate(loader):
        ...
        with torch.no_grad():
            ...
            # Calcular densidad actual para el orquestador
            if hasattr(model, 'adj_weights'):
            ...
            metrics_dict = {
                ...
                'memory_norm': 1.0,
                'L_score': 2.0,
                'density': current_density  # CRÍTICO: Pasar densidad al orquestador
            }
    ...
    # NUEVO: Calcular y retornar densidad actual
    if config.use_plasticity:
        with torch.no_grad():
            adj_w = torch.sigmoid(model.adj_weights)
            current_density = (adj_w > 0.5).float().mean().item()
            metrics['density'] = current_density  # Añadir al dict de métricas
    else:
        metrics['density'] = 1.0
