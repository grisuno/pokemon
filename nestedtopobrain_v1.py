...
class Config:
    ...
    # ORQUESTADOR PREFRONTAL - CONTROLES DINÁMICOS
    use_orchestrator: bool = True
    orchestrator_hidden_dim: int = 32
    orchestrator_state_dim: int = 8  # [loss, grad_norm, sparsity, entropy, batch_var, epoch, memory_norm, topology_health]
...
class AdaptiveCombinatorialComplexLayer(nn.Module):
    ...
    def forward(self, x_nodes, adjacency, incidence, adj_sparse=None, inc_sparse=None, 
        ...
        """
        FIX: Integra señales de control del Orquestador con bypass condicional
        ...
        with torch.amp.autocast(device_type='cuda', enabled=False):
            ...
            # Aplicar importancia de nodos controlada por Orquestador
            if self.config.use_adaptive_topology and hasattr(self, 'node_importance'):
                ...
                # Escala de importancia controlada por plasticidad
                if controls is not None:
            ...
            if self.config.use_mgf:
                ...
                if self.symbiotic:
                    # Symbiotic controlado por Orquestador
                    if controls is not None:
            ...
            if self.config.use_mgf:
                if self.layer_type == 'midbrain':
                    # FIX: Bypass condicional de Predictive Coding cuando defensa es baja
                    if controls is not None and controls.get('defense', 1.0) > 0.3:
                        processed = self.pc_cell(h0_agg, pred_nodes)
                    else:
                        processed = h0_agg  # Skip costoso PC si defensa baja
                else:
            ...
            return self.norm(out), entropy, new_state_node, new_state_cell
...
class TopoBrainV24(nn.Module):
    ...
    def initialize_memories(self, dataloader):
        ...
        with torch.no_grad():
            for i, (x, y) in enumerate(dataloader):
                ...
                x_l1, _, _, _ = self.layer1(x_embed, curr_adj, curr_inc, adj_s, inc_s)
    ...
    def calculate_ortho_loss(self, controls=None):
        ortho_loss = 0.0
        count = 0
        
        for layer_idx, layer in enumerate([self.layer1, self.layer2]):
            if hasattr(layer, 'symbiotic') and layer.symbiotic is not None:
                basis = layer.symbiotic.basis_atoms
                basis_sq = torch.mm(basis, basis.T)
                identity_basis = torch.eye(basis.size(0), device=basis.device)
                weight = self.config.ortho_weights[min(layer_idx, len(self.config.ortho_weights)-1)]
                
                if controls is not None:
                    symbiotic_gate = controls.get('symbiotic', 1.0) if isinstance(controls, dict) else 1.0
                    weight = weight * symbiotic_gate
                
                ortho_loss += weight * torch.norm(basis_sq - identity_basis, p='fro') ** 2
                count += 1
    ...
    def forward(self, x, prev_states=None, controls=None):
        """Forward con validación y detach explícito"""
        batch_size = x.size(0)
        ...
        with torch.amp.autocast(device_type='cuda', enabled=False):
            ...
            # VALIDAR Y LIMPIAR ESTADOS PREVIOS
            ...
            x_l1, entropy_l1, new_state_l1_node, new_state_l1_cell = self.layer1(
                x_embed, curr_adj, curr_inc, adj_s, inc_s, 
                prev_state_l1_node, prev_state_l1_cell, controls
            )
            
            # LAYER 2 CON ESTADES VALIDADOS
            ...
            x_l2, entropy_l2, new_state_l2_node, new_state_l2_cell = self.layer2(
            ...
            # DETACH NEW STATES BEFORE RETURNING
            new_states = {
            ...
            return logits, proj, total_entropy, new_states
    ...
    def prune_topology(self, controls=None):
        ...
        with torch.no_grad():
            ...
            if current_sparsity < target_sparsity:
                percentile = (1.0 - target_sparsity) * 100
                dynamic_threshold = torch.quantile(adj_w, percentile / 100.0).item()
            else:
            ...
            print(f"  ✂️  Pruning: Threshold={dynamic_threshold:.3f} | Target={target_sparsity:.1%} | " 
                f"Sparsity: {current_sparsity:.2%} → {final_sparsity:.2%} | Hubs preserved: {is_hub.sum().item()}")
...
def analyze_topology_flow(model, dataloader, run_name: str, num_samples=100):
    ...
    with torch.no_grad():
        for i, (x, y) in enumerate(dataloader):
            ...
            x_l1, _, _, _ = model.layer1(x_embed, curr_adj, curr_inc, adj_s, inc_s)
...
def make_adversarial_pgd(model, x, y, eps, steps, dataset_name, controls=None, prev_states=None):
    ...
    model.eval()  # Switch a modo evaluación para desactivar Dropout/BatchNorm stats updates
    
    # 1. Controles seguros (copia profunda para evitar efectos colaterales en tensores de control)
    safe_controls = None
    if controls is not None:
        safe_controls = {}
    ...
    # 2. CONGELAR PESOS DEL MODELO (Aislamiento Crítico)
    # Guardamos el estado original de requires_grad para restaurarlo después
    prev_grad_states = {}
    ...
    # 3. Setup Delta
    actual_steps = steps
    # Modulación dinámica de la profundidad del ataque basada en el gate de defensa
    if safe_controls is not None and 'defense' in safe_controls:
        ...
        # Si la defensa es baja, reducimos pasos para ahorrar energía computacional
    ...
    try:
        # 4. Bucle de ataque (Simulación mental adversaria)
        for step in range(actual_steps):
            # Grafo aislado: solo delta -> logits -> loss
            # Pasamos None en prev_states para no arrastrar memoria LSTM/GRU innecesaria en el ataque
            logits, _, _, _ = model(x + delta, None, safe_controls)
            loss = F.cross_entropy(logits, y)
            
            # Limpiar gradiente solo de delta
            if delta.grad is not None:
                delta.grad.zero_()
            
            # Backward solo genera gradientes para delta porque el modelo está congelado
            loss.backward()
            
            with torch.no_grad():
                if delta.grad is not None:
                    grad = delta.grad
                    # Normalización de gradiente para estabilidad numérica
        ...
        # 5. Restaurar plasticidad del modelo (Descongelar)
        for name, param in model.named_parameters():
...
def evaluate(model, loader, config, adversarial=False, controls=None):
    """
    Evaluación optimizada para arquitecturas biológicas complejas (Nested/Grid).
    ...
    # 1. Limpieza preventiva de memoria VRAM
    if torch.cuda.is_available():
        torch.cuda.empty_cache()
        torch.cuda.synchronize()
        
    # 2. Preparación de controles estáticos (sin grafo)
    eval_controls = None
    if controls is not None:
        eval_controls = {k: (v.detach() if torch.is_tensor(v) else v) for k, v in controls.items()}

    # 3. PROTOCOLO DE SEGURIDAD DE MEMORIA (Gradient Shielding)
    # Desactivamos requires_grad en TODOS los parámetros temporalmente.
    # torch.no_grad() a veces no es suficiente si hay operaciones complejas o hooks.
    prev_grad_states = {}
    for name, param in model.named_parameters():
        prev_grad_states[name] = param.requires_grad
        param.requires_grad = False
    ...
    try:
        for i, (x, y) in enumerate(loader):
            ...
            if adversarial:
                # Reactivar contexto de gradiente SOLO para la entrada (x)
                # Los pesos del modelo siguen congelados por el paso 3
                with torch.enable_grad():
                    x = make_adversarial_pgd(model, x, y, config.test_eps, 
                                            config.pgd_steps_test, config.dataset,
                                            controls=eval_controls)
            
            # Inferencia pura sin construcción de grafo
            with torch.no_grad():
                # Pasamos prev_states=None para resetear memoria episódica en test
                logits, _, _, _ = model(x, None, eval_controls)
                correct += logits.argmax(1).eq(y).sum().item()
                total += y.size(0)
            
            # Limpieza periódica agresiva durante evaluación
            if i % 10 == 0 and torch.cuda.is_available():
                torch.cuda.empty_cache()
                
    finally:
        # 4. Restaurar estado vital de gradientes
        for name, param in model.named_parameters():
            param.requires_grad = prev_grad_states[name]
...
def train_epoch(model, loader, optimizer, opt_topo, config, epoch, monitor, scaler=None, sparsity_lambda=None):
    """
    Entrenamiento homeostático con gestión rigurosa de grafos y memoria.
    ...
    # Estados persistentes intra-epoch (Memoria de corto plazo)
    epoch_states = None
    grad_norm_window = []
    ...
    for i, (x, y) in enumerate(loader):
        ...
        # --- A. ORQUESTADOR: PERCEPCIÓN DEL ESTADO ---
        controls = None
        metrics_dict = {}
        
        with torch.no_grad():
            # Decadencia de memoria de estados latentes para evitar saturación
            if i % 5 == 0 and epoch_states is not None:
                for k in epoch_states:
                    if epoch_states[k] is not None:
                        epoch_states[k].mul_(0.9)
            
            # Cálculo de métricas de entrada al orquestador
            ...
            metrics_dict = {
                ...
                'sparsity': sparsity,
                'entropy': 0.5, # Placeholder inicial
        ...
        # Costos Estructurales y Topológicos
        if controls is not None:
            # Ortogonalidad de bases simbióticas
            ortho = model.calculate_ortho_loss(controls)
            sym_gate = controls.get('symbiotic', 1.0)
            loss += config.lambda_ortho * ortho * sym_gate
            metrics['ortho_loss'] += ortho.item()
            
            # Diversidad topológica
            div_loss = model.calculate_topology_diversity_loss(controls)
            loss += 0.01 * div_loss
            metrics['topology_diversity'] += div_loss.item()
            
            # Penalización por densidad (Costo metabólico de conexiones)
            if config.use_plasticity and topology_warmup and sparsity_lambda is not None:
                ...
                # El orquestador decide cuánto penalizar la densidad
                loss += sparsity_lambda * sparsity_penalty * controls['plasticity']
                metrics['sparsity_loss'] += sparsity_penalty.item()
        
        # --- F. ACTUALIZACIÓN SINÁPTICA (BACKWARD) ---
        loss.backward()
        
        # --- G. OPTIMIZACIÓN Y HOMEOSTASIS ---
        if (i + 1) % config.accumulation_steps == 0:
            ...
            # Modulación dinámica del Learning Rate por el Orquestador
            ...
            # Monitorización de flujo de gradientes
            with torch.no_grad():
                total_norm = 0.0
        ...
        # --- H. ACTUALIZACIÓN DE MEMORIA DE CORTO PLAZO ---
        ...
        # --- I. VISUALIZACIÓN DE ESTADO COGNITIVO (Logs en Tiempo Real) ---
        if i % 50 == 0:
            ResourceMonitor.check_limit(config.memory_limit_gb)
            if controls is not None:
                # AQUÍ ESTÁ EL LOG DETALLADO QUE FALTABA
    ...
    # Normalización final de métricas por época
    n = len(loader)
