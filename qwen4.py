...
def train_nonstationary(config: Config):
    ...
    for step in range(config.steps):
        ...
        # 🔥 CORRECCIÓN CLAVE: convertir a float para evitar broadcasting error
        entropy_val = entropy.item() if torch.is_tensor(entropy) else float(entropy)
        ortho_val = ortho.item() if torch.is_tensor(ortho) else float(ortho)
        
        loss = criterion(logits, y)
        loss -= 0.01 * entropy_val
        loss += 0.05 * ortho_val
        optimizer.zero_grad()
        loss.backward()
