...
# Especialización hemisférica sin overhead innecesario
# Dataset: MS COCO Captions (real human annotations)
# =============================================================================
import torch
...
# =============================================================================
# LIQUID NEURON (de tu unicameral, sin cambios)
# =============================================================================
    ...
    def forward(self, x, global_plasticity=0.0, transfer_rate=0.005):
        ...
        if self.training and effective_plasticity.mean() > 0.001:
            with torch.no_grad():
                ...
                # REGULARIZACIÓN BALANCEADA (punto medio)
                self.W_fast.data.clamp_(-2.7, 2.7)
                if self.W_fast.norm() > 2.0:
                    self.W_fast.data *= 0.98
...
# =============================================================================
# HEMISFERIO DERECHO: Procesamiento Visual Espacial (ResNet pretrained)
# =============================================================================
class RightHemisphere(nn.Module):
    """
    Especialización: Visión espacial, reconocimiento de objetos, contexto global
    Usa ResNet-50 pretrained en ImageNet → Feature extraction robusto
    """
    def __init__(self, output_dim=512):
        super().__init__()
        # Cargar ResNet-50 pretrained
        resnet = models.resnet50(pretrained=True)
        
        # Congelar layers tempranos (features básicas ya aprendidas)
        for param in list(resnet.parameters())[:-20]:
            param.requires_grad = False
        
        # Extraer hasta avgpool (antes de FC)
        self.visual_encoder = nn.Sequential(*list(resnet.children())[:-1])
        
        # Spatial processing con liquid neuron
        self.spatial_liquid = LiquidNeuron(2048, output_dim)
        
        self.output_dim = output_dim
        
    def forward(self, image, plasticity=0.1, transfer_rate=0.005):
        # Extract visual features [B, 2048, 1, 1]
        features = self.visual_encoder(image)
        features = features.flatten(1)  # [B, 2048]
        
        # Procesar con liquid neuron (adaptación dinámica)
...
# =============================================================================
# HEMISFERIO IZQUIERDO: Procesamiento Secuencial y Lenguaje
# =============================================================================
...
# =============================================================================
# CORPUS CALLOSUM: Comunicación Interhemisférica Minimalista
# =============================================================================
...
def build_vocab(ann_file, vocab_size=10000):
    """Construir vocabulario desde annotations de COCO"""
    print("Building vocabulary...")
    ...
    # Top words más frecuentes
    most_common = counter.most_common(vocab_size - 4)  # -4 para tokens especiales
...
class LifeCycle:
    ...
    def get_plasticity(self, epoch):
        if epoch < 3:
            return 0.3  # Exploración inicial
        elif epoch < 20:
            return max(0.01, 0.15 * (1 - epoch/20))
        else:
            return 0.001  # Consolidación
...
def train_bicameral():
    ...
    for epoch in range(start_epoch, 30):
        ...
        temperature = max(0.8, 1.3 - epoch * 0.015)
