...
# =============================================================================
# GENERADOR DE AUDIOS (Edge-TTS)
# =============================================================================
async def generate_audio_async(text, output_path, voice="en-US-AriaNeural"):
    """Genera un audio usando Edge-TTS"""
    try:
        import edge_tts
        ...
        print(f"\n❌ Error generando {output_path}: {e}")
        return False


async def generate_all_audios_batch(captions_list, audio_dir, batch_size=100):
    """Genera todos los audios en batches para eficiencia"""
    ...
    for key, text in captions_list:
        ...
        if not os.path.exists(output_file):
            tasks.append((key, text, output_file))
    
    if len(tasks) == 0:
        print("✓ Todos los audios ya existen")
        return
    
    print(f"🎵 Generando {len(tasks)} audios con Edge-TTS...")
    print(f"   Esto tomará ~{len(tasks) // 40} minutos aproximadamente")
    ...
    for i in range(0, len(tasks), batch_size):
        ...
        for task in tqdm(asyncio.as_completed(batch_tasks), 
                        ...
                        desc=f"Batch {i//batch_size + 1}/{(len(tasks)-1)//batch_size + 1}"):
            result = await task
    ...
    print(f"✅ Audios generados: {len(tasks)} archivos")
    ...
    try:
        ...
        if "already running" in str(e):
            ...
            try:
                ...
                print("⚠️  Instalando nest_asyncio para Jupyter/Colab...")
...
# =============================================================================
# UTILIDADES: Vocabulario y funciones auxiliares
# =============================================================================
def setup_flickr8k(data_dir='./flickr8k_full'):
    """Descarga y organiza Flickr8k si no existe"""
...
def train_tricameral():
    ...
    # Setup dataset (descarga si no existe)
    flickr_dir = setup_flickr8k('./flickr8k_full')
    ...
    if not os.path.exists(audio_dir) or len(os.listdir(audio_dir)) < 1000:
        print("⚠️  Audios no encontrados. Generando automáticamente...\n")
        
        # Verificar si edge-tts está instalado
        try:
            import edge_tts
        except ImportError:
        ...
        if os.path.exists(audio_dir) and len(os.listdir(audio_dir)) >= 1000:
            print(f"✅ Audios generados: {len(os.listdir(audio_dir))} archivos")
            use_audio = True
        else:
            print("⚠️  No se pudieron generar suficientes audios. Continuando en modo bimodal.")
            use_audio = False
    else:
