...
# Mejoras: Estabilidad del Gate, Control Callosal, LR y Plasticidad Suave
# =============================================================================
import torch
...
# =============================================================================
# DESCARGA AUTOMÁTICA DE FLICKR8K (Código de v1.1)
# =============================================================================
def setup_flickr8k(data_dir='./data'):
    ...
    if os.path.exists(raw_captions):
        ...
        # Mover directorio de imágenes si es necesario
        ...
        print(f"✓ Captions y directorios listos\n")
    
    print("✅ Flickr8k listo\n")
    return flickr_dir

# =============================================================================
# LIQUID NEURON (Ajustado v2.0: clamp, base_lr, plasticidad)
# =============================================================================
class LiquidNeuron(nn.Module):
    def __init__(self, in_dim, out_dim):
        ...
        self.base_lr = 0.01 # Reducido de 0.015
    ...
    def forward(self, x, global_plasticity=0.0, transfer_rate=0.005):
        ...
        # Factor de escalado reducido a 3.0 para evitar la saturación
        ...
        if self.training:
            ...
            # Plasticidad efectiva modulada por el error de predicción
            ...
            if effective_plasticity.mean() > 0.001:
                with torch.no_grad():
                    ...
                    self.W_fast.data.clamp_(-2.0, 2.0) # Clamp más estricto

        return out
    
    # Consolidación SVD de v1.1
...
# =============================================================================
# HEMISFERIO DERECHO (sin cambios)
# =============================================================================
...
# =============================================================================
# CORPUS CALLOSUM (mejorado con Gating)
# =============================================================================
class CorpusCallosum(nn.Module):
    def __init__(self, dim=512):
        super().__init__()
        self.right_to_left = nn.Linear(dim, dim)
        
        # Gate de atención basado en la señal visual
        self.callosal_gate = nn.Sequential(
            nn.Linear(dim, dim // 4),
            nn.ReLU(),
            nn.Linear(dim // 4, dim),
            nn.Sigmoid() 
    ...
    def forward(self, right_features):
        projected = self.right_to_left(right_features)
        gate = self.callosal_gate(right_features) 
        gated_features = projected * gate
        return self.layer_norm(gated_features)
...
class LeftHemisphere(nn.Module):
    def __init__(self, vocab_size, embed_dim=256, hidden_dim=512):
        ...
        # Gate recibe (Hidden State + Context Norm + Error) = hidden_dim + 2
    ...
    def forward(self, visual_context, captions=None, max_len=30, prediction_error=0.0, return_gate=False):
        ...
        if captions is not None:
            ...
            context_norm = visual_context.norm(dim=-1, keepdim=True)
            ...
            # Entrada al Gate: Hidden State + Context Norm + Error
            error_tensor = torch.full((batch_size, seq_len, 1), prediction_error, device=device)
            norm_expanded = context_norm.unsqueeze(1).expand(-1, seq_len, -1)
            
            gate_input = torch.cat([lstm_out, norm_expanded, error_tensor], dim=2) 
            
            gate = self.liquid_gate(gate_input) 
            
            # Modulación con factor de escalado (0.5 * G + 0.5) para evitar el apagón total
            modulated = lstm_out * (0.5 * gate + 0.5) 
        ...
        # Lógica de generación (Inferencia)
        else:
            generated = []
            ...
            for step in range(max_len):
                ...
                # Inferencia: Error de Predicción = 0.0
                context_norm = visual_context.norm(dim=-1, keepdim=True).unsqueeze(1)
                error_tensor = torch.zeros_like(context_norm) 
                
                gate_input = torch.cat([out, context_norm, error_tensor], dim=2)
                gate = self.liquid_gate(gate_input)
                
                out = out * (0.5 * gate + 0.5)
...
# =============================================================================
# NEUROLOGOS BICAMERAL (Actualizado para pasar el error)
# =============================================================================
class NeuroLogosBicameral(nn.Module):
    ...
    def forward(self, image, captions=None, plasticity=0.1, transfer_rate=0.005, return_diagnostics=False):
        ...
        # Necesitamos el error de predicción del hemisferio derecho
        prediction_error = self.right_hemisphere.spatial_liquid.prediction_error
        
        if return_diagnostics and captions is not None:
            output, gate = self.left_hemisphere(
                visual_context, captions, 
                prediction_error=prediction_error, 
                return_gate=True
            )
            return output, visual_features, visual_context, gate
        
        output = self.left_hemisphere(visual_context, captions, prediction_error=prediction_error)
        return output

# =============================================================================
# DIAGNOSTICO NEUROLOGICO (de v1.1)
# =============================================================================
...
# =============================================================================
# FLICKR8K DATASET / VOCAB (de v1.1)
# =============================================================================
...
# =============================================================================
# LIFECYCLE (decay más suave v2.0)
# =============================================================================
class LifeCycle:
    def __init__(self, total_epochs=TOTAL_EPOCHS):
    ...
    def get_plasticity(self, epoch):
        if epoch < 5:
            ...
            return 0.25 * np.exp(-0.1 * (epoch - 5)) 
        else:
            return 0.005 

# =============================================================================
# TRAINING OPTIMIZADO (Ajuste LR a 1e-4)
# =============================================================================
def train_bicameral():
    ...
    # LR reducido para mayor estabilidad con RNN
    optimizer = torch.optim.AdamW(model.parameters(), lr=1e-4, weight_decay=1e-5)
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=TOTAL_EPOCHS)
    
    life = LifeCycle(total_epochs=TOTAL_EPOCHS)
    ...
    for epoch in range(start_epoch, TOTAL_EPOCHS):
        ...
        # Tasa de transferencia ligeramente reducida en v2.0
        transfer_rate = 0.005 if epoch < 20 else 0.003
        ...
        for batch_idx, (images, captions) in enumerate(pbar):
            ...
            # Actualiza el error de predicción en el LiquidNeuron
            with torch.no_grad():
            ...
            with torch.no_grad():
                ...
                # Se mide la plasticidad aprendida con las estadísticas del batch actual
                ...
                learned_plast = liquid.plasticity_controller(batch_stats).mean().item()
        ...
        if epoch % 2 == 0:
            ...
            with torch.no_grad():
                # Toma muestras espaciadas
                indices = [i * 100 for i in range(3)] 
                for sample_idx, idx in enumerate(indices):
                    # Asegurarse de que el índice no exceda el dataset
                    if idx >= len(dataset): continue 
                    
                    sample_img, sample_cap = dataset[idx]
