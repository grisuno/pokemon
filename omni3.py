#!/usr/bin/env python3
# =============================================================================
# OMNI BRAIN v8.1 - CIFAR-10 CON SISTEMA DE ABLACIÓN Y LOGGING PROFESIONAL
# =============================================================================
import torch
...
class Config:
    ...
    # HIPERPARÁMETROS - AJUSTADOS PARA EFICIENCIA EN CPU
...
class FastSlowLinear(nn.Module):
    def __init__(self, in_features, out_features, config: Config):
        ...
        # Pesos lentos (backprop standard) - SOLO INICIALIZACIÓN XAVIER
        self.slow_weight = nn.Parameter(torch.empty(out_features, in_features))
        self.slow_bias = nn.Parameter(torch.zeros(out_features))
        nn.init.xavier_uniform_(self.slow_weight)  # Eliminada inicialización redundante
...
class IntegrationModule(nn.Module):
    ...
    def __init__(self, features: int, config: Config):
        super().__init__()
        self.config = config  # FIX: Almacena referencia para ablación dinámica
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # FIX: Usa self.config en lugar de config global para ablación correcta
...
class OmniBrainFastSlow(nn.Module):
    ...
    def reset_all_fast_weights(self):
        """
        Reinicia todos los pesos rápidos del modelo. 
...
def train(config: Config):
    logger.info("🔬 OMNI BRAIN v8.1 - MODO EXPERIMENTAL CON ABLACIÓN (FULLY FIXED)")
    logger.info("=" * 80)
    ...
    for epoch in range(config.epochs):
        ...
        # FIX: Reset de pesos rápidos cada 10 épocas (no cada época) para preservar memoria
...
def run_ablation_study():
    ...
    logger.info(f"\n📊 RESULTADOS FINALES DE ABLACIÓN:")
...
if __name__ == "__main__":
    ...
    # Para estudio de ablación completo (descomentar después de single run)
    # run_ablation_study()