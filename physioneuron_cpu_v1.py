...
===============================================================================================
Misión: Agregar el mecanismo de Physio-Chimera v14 (regulación interna de metabolismo, sensibilidad y gating)
       al marco de ablación existente de NeuroLogos v5.1, sin eliminar ni modificar ningún componente.

Nuevo componente:
- use_homeostasis: cada nodo se convierte en una PhysioNeuron con regulador interno
...
# =============================================================================
# CONFIGURACIÓN CIENTÍFICA CPU-FRIENDLY (agregado: use_homeostasis)
# =============================================================================
@dataclass
class MicroConfig:
    device: str = "cpu"
    ...
    use_homeostasis: bool = False  # ← NUEVO COMPONENTE
...
# =============================================================================
# NUEVO: PHYSIONEURON (copiado fielmente de Physio-Chimera v14)
# =============================================================================
...
# =============================================================================
# MICROTOPOBRAIN v5.2 — CON INTEGRACIÓN DE PHYSIONEURON
# =============================================================================
class MicroTopoBrain(nn.Module):
    def __init__(self, config: MicroConfig):
        ...
        # NUEVO: si use_homeostasis=True → PhysioNeuron; else → continuum o linear
        if config.use_homeostasis:
            self.node_processor = PhysioNeuron(self.embed_dim, self.embed_dim, dynamic_mode=True)
    ...
    def forward(self, x, plasticity=1.0):
        ...
        # NUEVO: soporte para PhysioNeuron
        if isinstance(self.node_processor, PhysioNeuron):
...
# =============================================================================
# ENTRENAMIENTO Y EJECUCIÓN (sin cambios)
# =============================================================================
...
def run_ablation_study():
    ...
    print("="*80)
    print("🧠 NeuroLogos v5.2 - TopoBrain con Control Homeostático Fisiológico")
    print("="*80)
    print("✅ Se añade 'homeostasis' como componente nuevo en la ablación 3-niveles")
    print("✅ Sin eliminar ni modificar ningún componente existente")
    print("="*80 + "\n")
    ...
    print("\n" + "="*80)
    print("📊 RESULTADOS - ¿ES SUPERIOR LA HOMEOSTASIS?")
    print("="*80)
    # Encontrar mejor sin homeostasis
    no_homeo = {k: v for k, v in results.items() if 'Homeostasis' not in k}
    best_no_homeo = max(no_homeo.items(), key=lambda x: x[1]['pgd_mean'])

    # Encontrar mejor con homeostasis
    with_homeo = {k: v for k, v in results.items() if 'Homeostasis' in k}
    best_with_homeo = max(with_homeo.items(), key=lambda x: x[1]['pgd_mean'])

    print(f"{'Modelo':<40} {'PGD Acc':<12} {'Clean Acc':<12} {'Params'}")
    print(f"{best_no_homeo[0]:<40} {best_no_homeo[1]['pgd_mean']:>6.2f}%     {best_no_homeo[1]['clean_mean']:>6.2f}%     {best_no_homeo[1]['n_params']:,}")
    print(f"{best_with_homeo[0]:<40} {best_with_homeo[1]['pgd_mean']:>6.2f}%     {best_with_homeo[1]['clean_mean']:>6.2f}%     {best_with_homeo[1]['n_params']:,}")
